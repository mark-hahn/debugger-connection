...
function _doResponse(connection, data) {
  ...
  if (!seq) {
    requests = _.filter(connection._fnPool, function(request) {
      return request.command = data.command;
    });
  }
  else {
    requests = [connection._fnPool[seq]];
  }
    ...
    function checkIsDone(request) {
      return function() {
        ...
        if (!request.done) {
          ...
          connection._fnPool[request.seq] = request;
          ...
          if (request.seqHistory.length === 10) {
            ...
            next(connection);
            return request.callback(new Error('Request timeout for request:' + request.command));
          }
        }

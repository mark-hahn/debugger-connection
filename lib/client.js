...
function Client(port) {
  ...
  function ondata(data) {
    ...
    data.split('\r\n\r\n').forEach(function(block) {
      try {
        block = utils.camelize(JSON.parse(block))
...
Client.prototype.request = function(command, args, opts, cb) {
  var seq = this.seq;
  var noTrainsform = false;
  var data = {
    seq: seq,
    type: 'request',
    command: command,
  ...
  if (opts) {
    noTrainsform = opts.noTrainsform
  }

  data = noTrainsform ? data : utils.lodashlize(data)

  var serilizedData = this.protocol.serilize(data);
  this._fnPool[seq] = cb;
